...
/**
 * @description - 安全服务类
 * @oldname - toolkit/security/securityService.php
 * @newname - ext.s.service.php
...
class S {
	...
	public static function getHtmlGP($key) {
		$content = S::getGP ( $key );
	...
	public static function reEscapeStr($string) {
		$string = str_replace ( array ('&lt;' ), '<', $string );
		$string = str_replace ( array ('&gt;' ), '>', $string );
		$string = str_replace ( array ('&quot;', '&#39;', '&nbsp;' ), array ('"', "'", ' ' ), $string );
		$string = preg_replace ( "/(<(?:script|script\s[^>]*)>)([\s\S]*?)(<\/script>)/i", '', $string );
		$string = preg_replace ( "/(<(?:style|style\s[^>]*)>)([\s\S]*?)(<\/style>)/i", '', $string );
		return $string;
