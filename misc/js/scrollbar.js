...
Scrolling.Scroller = function (o, w, h, t, type) {
	//get the container
	...
	function setPosition (x, y) {
		...
		if(y < (_vheight - _theight) + 20){ //CallBack to Load NextPage
			ShowBoxScroll.ScrollCallBack(y, type);
		}
...
CM.intro = {
	...
	scrollbar : {},
	tween : {},
	start: function(DvScrollId, DvScrollBarId, type) {
		if ($('#' + DvScrollId).length) CM.intro.iniScr(DvScrollId, DvScrollBarId, type);
	},
	iniScr: function(DvScrollId, DvScrollBarId, type){
		if(!CM.intro.tween[type]) CM.intro.tween[type] = new Scrolling.ScrollTween();
		scroller  = new Scrolling.Scroller(document.getElementById(DvScrollId), 960, 450, CM.intro.tween[type], type);
		scrollbar = new Scrolling.Scrollbar(document.getElementById(DvScrollBarId), scroller, CM.intro.tween[type]);
		CM.intro.scroller[type] = scroller;
		CM.intro.scrollbar[type] = scrollbar;
	},
	reset: function(DvScrollId, DvScrollBarId, type, y){
		CM.intro.scroller[type].reset();
		CM.intro.scrollbar[type].reset();
		CM.intro.scrollbar[type].scrollTo(-y);
	}
};

////////////////////  start  ////////////////////
$(function() {
	//CM.intro.start(ObjDvScroll, ObjDvScrollBar);
});