...
class AdminController extends BaseController {
	...
	public function productseq() {
		...
		if (intval ($step) == 2) {
			...
			if (! S::isArray ($seq))
				$this->jsonOutput ($callback, array (false,'信息未填写完整' ));
			foreach ( $seq as $id => $v ) {
				$this->getModelFactory ()->getProductModel ()->update ($id, array ('seq' => intval ($v) ));
			}
			$this->jsonOutput ($callback, array (true,'顺序修改成功' ));
		}
	...
	public function productdelete() {
		...
		if (intval ($step) == 2) {
			...
			if ($aid == '')
				$this->jsonOutput ($callback, array (false,'请选择欲操作对象' ));
			list ( $bool ) = $this->getHelperFactory ()->getProductHelper ()->deleteProducts ($aid);
			$this->jsonOutput ($callback, $bool ? array (true,'' ) : array (false,'操作失败，请重试' ));
		}
		$this->jsonOutput ($callback, array (false,'异常操作' ));
	}
	...
	public function productadd() {
		list ( $step ) = S::gp (array ('step' ));
		if (intval ($step) == 2) {
			list ( $callback, $pid, $title, $status, $cover, $coverhover, $smallpic_seq, $smallpic, $smallpicFlag ) = S::gp (array ('callback','pid','title','status','cover','coverhover','smallpic_seq','smallpic','smallpic_flag' ));
			list ( $pid, $title, $status, $cover, $coverhover, $smallpicFlag ) = array (intval ($pid),trim ($title),intval ($status),trim ($cover),trim ($coverhover),intval ($smallpicFlag) );
			if ($title == '')
				$this->jsonOutput ($callback, array (false,'名称不能为空' ));
			$fields = array ('category' => intval ($pid),'title' => $title,'status' => $status,'cover' => $cover,'coverhover' => $coverhover );
			$result = $this->getHelperFactory ()->getProductHelper ()->addProduct ($fields, $smallpicFlag, $smallpic_seq, $smallpic);
	...
	public function productmodify() {
		...
		if (intval ($step) == 2) {
			...
			if ($title == '')
				$this->jsonOutput ($callback, array (false,'名称不能为空' ));
			$fields = array ('category' => intval ($pid),'title' => $title,'status' => $status,'cover' => $cover,'coverhover' => $coverhover );
			$result = $this->getHelperFactory ()->getProductHelper ()->updateProduct ($id, $fields, $smallpicFlag, $smallpic_seq, $smallpic);
	...
	public function managermodify() {
		...
		if (intval ($step) == 2) {
			...
			($password && $password2) && $fields ['userpsw'] = md5 ($password2);
	...
	public function removefile() {
		list ( $sign, $type, $field, $callback ) = S::gp (array ('sign','dir','field','callback' ));
		$this->_userInfo = $this->getAdminModel ()->isLogin ();
		if (! S::isArray ($this->_userInfo)) {
			$this->jsonOutput ($callback, array ('error' => 1,'message' => '请先登录' ));
		}
		if (! $sign || ! $field) {
			$this->jsonOutput ($callback, array ('error' => 1,'message' => '参数错误' ));
		}
		$bool = $this->getModelFactory ()->getCommonUploadModel ()->deleteFile ($field, $sign);
		$this->jsonOutput ($callback, array ('error' => 0,'message' => $bool ));
	}
